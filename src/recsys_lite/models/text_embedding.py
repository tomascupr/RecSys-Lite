...
class TextEmbeddingModel(BaseRecommender):
    ...
    def __init__(
        ...
        item_text_fields: List[str] = ["title", "category", "brand", "description"],
    ...
    def fit(self, user_item_matrix: sp.csr_matrix, **kwargs: Any) -> None:
        ...
        if self.cache_embeddings and output_dir:
            ...
            if embedding_path.exists() and ids_path.exists():
                try:
                    ...
                    logger.info(f"Loaded embeddings for {len(self.item_ids)} items")
                    return
        ...
        # Combine batches
        embeddings = np.vstack(all_embeddings).astype(np.float32)  # Use float32 for memory efficiency
        
        # Normalize if requested and not already normalized by the model
        if self.normalize_vectors and not self.model.normalize_embeddings:
    ...
    def recommend(
        ...
        **kwargs: Any,
        ...
        item_indices = [self.id_to_idx[item_id] for item_id in interacted_item_ids 
        ...
        filtered_indices = [idx for idx in sorted_indices 
                           if self.item_ids[idx] not in interacted_item_ids]
