...
class HybridModel(BaseRecommender):
    ...
    def __init__(
        self,
        models: List[BaseRecommender] = None,
    ...
    def recommend(
        ...
        **kwargs: Any,
        ...
        if logger.isEnabledFor(logging.DEBUG):
            weight_info = ", ".join([f"{m.model_type}:{w:.2f}" for m, w in zip(self.models, weights)])
        ...
        for model, weight in zip(self.models, weights):
            ...
            for idx, score in zip(item_ids, normalized_scores):
                curr_score = all_item_scores.get(idx, 0)
                # Use max aggregation for scores from different models
                all_item_scores[idx] = max(curr_score, score * weight)
        ...
        item_ids, scores = zip(*top_items)
        return np.array(item_ids, dtype=np.int_), np.array(scores, dtype=np.float32)
            ...
            if self.cold_start_strategy == "content_boost" and content_indices and collab_indices:
                ...
                if total_content_weight > 0:
                    for i in content_indices:
                        ...
                        adjusted_weights[i] += transfer * (adjusted_weights[i] / total_content_weight)
